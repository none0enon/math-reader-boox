...
This repository contains the Math Reader PWA and Android app, including its WebView host, durable recording bridge, and BOOX/E Ink adaptation. AI services and R2 sync are configured by the user, and supported BOOX devices use the Onyx pen SDK for lower-latency handwriting.
...
3. 进入具体场次后，使用右下角按钮：
   ...
   - **相机**：拍摄板书、讲义或现场资料。
4. 上传后资料保存在“源素材”。图片和录音可调用配置的多模态模型生成 AI 笔记；停止录音后，应用会保存录音并逐段生成转写/纪要。
...
4. Uploaded material appears under Source Material. Images and recordings can use the configured multimodal model to generate AI notes; stopping a recording saves it and generates transcript/minutes in segments.
