...
长录音（80~90 分钟及以上）的转写纪要按 5 分钟一段依次提交给 AI，分段过程中会显示“正在准备录音分段”的进度提示，界面保持可用。若个别分段没能转写，纪要仍会保存，并在对应位置标注该段转写失败。网页版（GitHub Pages）在浏览器内切分长录音受内存限制，超过上限时会提示改用安装版 App 生成。
