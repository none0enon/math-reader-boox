...
| 模块 | 用途 |
|---|---|
| 课堂 | 按课程/讲座整理场次，保存资料和耐久录音；图片与录音可交给配置的多模态模型生成笔记 |
| 书架与阅读 | 管理书籍/论文，阅读和批注 PDF，搜索/OCR、套索问 AI、按文档保存对话 |
| 讲义 | 为 PDF 书籍生成大纲和分章节讲义；根据论文卡片的标题、作者和已有摘要生成新摘要 |
...
> AI 生成、OCR、录音纪要、识题和批改会把相应文本、页面截图、PDF、图片或录音发送给你配置的服务商。普通阅读、手写和本地数据管理不要求配置 AI；课堂中的普通 PDF/Word/TXT/Markdown/TeX 附件目前只保存文件，生成笔记时不会把文件内容加入模型请求。
...
5. 首次使用可按需完成 AI 和备份设置；当前 APK 会把 **设置 → 存储持久化** 状态固定显示为成功，不能把该状态当作系统真实授权结果。
...
1. 在 **Primary API** 选择 DeepSeek、OpenAI、Google、Claude 或 Custom；这些名称不代表所有调用路径都兼容，Claude 仅 PDF 附件有专用分支，Custom 当前按 Gemini 协议发送。
2. 填写服务地址、API Key 和模型名；兼容的服务可以使用 **Fetch Models** 拉取模型列表。
3. 如需故障切换，可再配置 **Backup API**。主服务调用失败时应用会尝试备用服务。
4. **Supabase** 和 **Mathpix** 位于 Other Services，但当前主流程没有接入 Supabase，Mathpix 也没有业务请求实现，可保持留空。
...
- **生成摘要**：根据论文卡片的标题、作者和已有摘要生成摘要；当前不会读取论文 PDF 正文。
...
- 搜索按钮支持关键词、上一项/下一项、区分大小写；扫描版 PDF 可对当前页或当前可见页使用 **OCR 识别搜索**，换文档后 OCR 缓存会清空。
...
3. 进入具体场次后，使用右下角按钮：
   ...
   - **相机**：拍摄板书、讲义或现场资料。
4. 上传后资料保存在“源素材”。图片会作为图像生成 AI 笔记；普通 PDF/Word/TXT/Markdown/TeX 附件目前不会把内容送入模型。停止录音后，应用会保存录音并调用配置的音频多模态模型逐段生成转写/纪要，不是内置 STT。
...
**生成论文摘要**：在 **书架 → AI → 生成摘要** 选择论文。当前请求只包含论文卡片的标题、作者和已有摘要，不读取论文 PDF 正文；结果显示在 **讲义 → 摘要** 栏，并使用同一讲义阅读器显示、渲染公式和保存阅读进度。
...
5. Configure AI and backups as needed. In the current APK, **Storage Persistence** is patched to report success unconditionally, so that status is not proof of a real browser grant.
...
1. Select DeepSeek, OpenAI, Google, Claude, or Custom as the primary provider. These labels do not guarantee compatibility on every path: only Claude PDF attachments use a dedicated Claude branch, while Custom currently uses the Gemini protocol.
...
4. Supabase and Mathpix appear under Other Services, but Supabase is not called by the active workflow and Mathpix has no business request implementation; leave them empty.
...
- **Generate Abstract** from the paper card's title, author, and existing abstract; the current implementation does not read the paper PDF body.
...
- Search supports previous/next result and case sensitivity. **OCR Search** processes the current or currently visible PDF pages, and its cache is cleared when the document changes.
...
4. Uploaded material appears under Source Material. Images are sent as images for AI notes, but ordinary PDF/Word/text/Markdown/TeX attachments are currently stored without sending their contents to the model. Stopping a recording saves it and sends audio segments to the configured multimodal model for transcript/minutes generation; this is not built-in STT.
...
For a paper, run **Generate Abstract**. The request currently uses only the paper card's title, author, and existing abstract—not the paper PDF body. The result appears in the Abstracts column and uses the same math-aware viewer.
...
2. 注入的 `boox-pen.js` 检测与当前页面 DOM 约定绑定的已知画布，把可见区域交给原生 `TouchHelper` 做 EPD 低延迟直渲染。
