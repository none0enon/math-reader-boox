...
test('full backup reports a missing recording and still exports every available payload', async () => {
    const html = readIndexHtml();
    const exportSource = sliceSource(html,
        'async function exportData()',
        'async function importDataZip(');
    const missing = {
        id: 'recording-missing', name: 'Missing recording', ref: 'R1',
        type: 'recording', mimeType: 'audio/webm', recordingStatus: 'external-backup'
    };
    const available = {
        id: 'recording-available', name: 'Available recording', ref: 'R2',
        type: 'recording', mimeType: 'audio/webm', duration: 12
    };
    const photo = { id: 'photo-available', name: 'Photo', type: 'image' };
    const availableBlob = new Blob(['saved-audio'], { type: 'audio/webm' });
    let zipFiles = null;
    let clicks = 0;
    const toasts = [];
    const liveClassroom = {
        courses: [{
            id: 'course',
            sessions: [{
                id: 'session', sourceFolder: [missing, available, photo],
                notes: [{ id: 'note', content: 'generated note remains available' }]
            }]
        }],
        seminars: []
    };
    const liveBefore = plain(liveClassroom);
    const { fn: exportData } = loadIsolatedFunction(exportSource, 'exportData', {
        Blob,
        ArrayBuffer,
        classroomRecordingState: null,
        appData: {
            books: [], papers: [], archived: [], lectures: {}, settings: {},
        ...
        _prepareRecordingExport: async material => {
            if (material.id === missing.id) throw new Error('recording bytes are unavailable');
            return {
                source: { kind: 'legacy', complete: true },
                byteLength: availableBlob.size,
                mimeType: availableBlob.type,
                idbEntries: null,
                chunks: [{ data: availableBlob, size: availableBlob.size, sha256: 'audio-hash' }],
                incomplete: false
            };
        },
        listFileKeys: async () => [
            '__recording_manifest_' + missing.id,
            '__recording_chunk_' + missing.id + '_00000000',
            'ordinary-idb-entry'
        ],
        getFileData: async key => key === 'ordinary-idb-entry' ? 'ordinary value' : { corrupt: true },
        dataZipCreate: async files => {
            zipFiles = files;
    ...
    assert.equal(clicks, 1, 'the backup download must still start');
    assert.ok(zipFiles, 'the ZIP must still be created');
    assert.equal(toasts.some(message => message.startsWith('导出失败:')), false);
    assert.match(toasts.at(-1), /1 条录音未包含音频/);
    assert.deepEqual(liveClassroom, liveBefore, 'export warnings must not mutate live app data');

    const metadata = JSON.parse(zipFiles.find(file => file.name === 'metadata.json').data);
    const exportedMaterials = metadata.classroom.courses[0].sessions[0].sourceFolder;
    assert.deepEqual(exportedMaterials.map(item => item.id), [available.id, photo.id],
        'an unavailable recording card must not make the restored backup inconsistent');
    assert.equal(metadata.classroom.courses[0].sessions[0].notes.length, 1,
        'generated notes remain available even when their source recording is missing');
    assert.equal(exportedMaterials[0].recordingDataIncluded, true);
    assert.equal(exportedMaterials[0].recordingBackupIncomplete, false);
    assert.deepEqual(metadata.exportWarnings.missingRecordings.map(item => item.id), [missing.id]);

    const idbIndex = JSON.parse(zipFiles.find(file => file.name === 'idb/index.json').data);
    assert.deepEqual(idbIndex.map(item => item.id), ['ordinary-idb-entry'],
        'unverified recording journals must not be restored as active data');

    const portable = JSON.parse(zipFiles.find(file => file.name === 'recordings/index.json').data);
    assert.deepEqual(portable.map(item => item.id), [available.id]);
    const report = JSON.parse(zipFiles.find(file => file.name === 'recordings/missing.json').data);
    assert.deepEqual(report.items.map(item => item.id), [missing.id]);
    assert.equal(report.items[0].material.id, missing.id,
        'the warning report retains non-binary metadata for manual recovery');
});

test('full backup aborts if recording starts during asynchronous preflight', async () => {
    const html = readIndexHtml();
    const exportSource = sliceSource(html,
        'async function exportData()',
        'async function importDataZip(');
    const material = { id: 'recording-preflight-race', type: 'recording' };
    const toasts = [];
    let isolatedContext;
    const loaded = loadIsolatedFunction(exportSource, 'exportData', {
        classroomRecordingState: null,
        appData: {
            classroom: {
                courses: [{ sessions: [{ sourceFolder: [material] }] }],
                seminars: []
            }
        },
        saveExercisesToIDB: async () => {},
        ...
        _prepareRecordingExport: async () => {
            isolatedContext.classroomRecordingState = { id: 'recording-started-mid-export' };
            return { source: { kind: 'legacy' }, chunks: [], incomplete: false };
        },
        showToast: message => toasts.push(message),
        console: { warn: () => {}, error: () => {} }
    });
    isolatedContext = loaded.context;

    await loaded.fn();

    assert.equal(toasts.at(-1), '导出失败: 请先停止当前录音，再创建一致的完整备份');
});

test('full backup omits and reports a recording added after its session preflight', async () => {
    const html = readIndexHtml();
    const exportSource = sliceSource(html,
        'async function exportData()',
        'async function importDataZip(');
    const available = { id: 'recording-preflight-existing', type: 'recording', name: 'Existing' };
    const late = { id: 'recording-preflight-late', type: 'recording', name: 'Late' };
    const earlySession = { id: 'early', sourceFolder: [] };
    const laterSession = { id: 'later', sourceFolder: [available] };
    const classroom = {
        courses: [{ id: 'course', sessions: [earlySession, laterSession] }],
        seminars: []
    };
    let zipFiles = null;
    let clicks = 0;
    ...
    const { fn: exportData } = loadIsolatedFunction(exportSource, 'exportData', {
        ...
        stripExercisesData: value => value,
        _prepareRecordingExport: async () => {
            earlySession.sourceFolder.push(late);
            return {
                source: { kind: 'legacy', complete: true },
                byteLength: audio.size,
                mimeType: audio.type,
                idbEntries: null,
                chunks: [{ data: audio, size: audio.size, sha256: 'audio-hash' }],
                incomplete: false
            };
        },
        listFileKeys: async () => [],
        getFileData: async () => null,
    ...
    assert.deepEqual(metadata.classroom.courses[0].sessions[0].sourceFolder, [],
        'a late metadata card must not be exported without a preflighted body');
    assert.deepEqual(metadata.classroom.courses[0].sessions[1].sourceFolder.map(item => item.id),
        [available.id]);
    const report = JSON.parse(zipFiles.find(file => file.name === 'recordings/missing.json').data);
    assert.deepEqual(report.items.map(item => item.id), [late.id]);
    assert.match(report.items[0].reason, /导出过程中新增/);
    assert.deepEqual(earlySession.sourceFolder.map(item => item.id), [late.id],
        'the live classroom is not mutated by backup filtering');
});
...
test('R2 recording manifest validation rejects mismatched or inconsistent generations', async () => {
    const html = readIndexHtml();
    const helperSource = sliceSource(html,
        'async function r2GetRecordingManifest(material)',
        'async function r2RestoreRecording(');
    const material = { id: 'recording-cloud-validation' };
    let remote;
    const { fn: getManifest } = loadIsolatedFunction(helperSource, 'r2GetRecordingManifest', {
        r2GetObject: async () => JSON.stringify(remote)
    });
    const valid = {
        format: 'recording-r2-groups-v1',
        id: material.id,
        byteLength: 5,
        groups: [{ key: 'recordings/group-0', size: 5 }]
    };

    remote = valid;
    assert.deepEqual(plain(await getManifest(material)), valid);
    for (const malformed of [
        { ...valid, id: 'recording-other-generation' },
        { ...valid, byteLength: 'bad' },
        { ...valid, groups: [] },
        { ...valid, byteLength: 6 }
    ]) {
        remote = malformed;
        await assert.rejects(getManifest(material), /云端录音清单格式无效/);
    }
});
...
test('ZIP import preserves incomplete recordings and clears reported missing bodies', async () => {
    const html = readIndexHtml();
    const importSource = sliceSource(html,
        'async function importDataZip(event)',
        'async function clearAllData()');
    const missingId = 'recording-missing-on-device';
    const incompleteId = 'recording-incomplete-import';
    const incompleteMaterial = { id: incompleteId, type: 'recording' };
    const chunk = new Blob(['abc'], { type: 'audio/webm' });
    const entries = {
        'metadata.json': new Blob([JSON.stringify({
            exportFormat: 'math-reader-full-zip-v2',
            classroom: {
                courses: [{ sessions: [{ sourceFolder: [incompleteMaterial] }] }],
                seminars: []
            }
        })]),
        'idb/index.json': new Blob([JSON.stringify([{
            id: 'classroom_' + missingId,
            path: 'idb/data/00000000.bin',
            kind: 'text'
        }])]),
        'idb/data/00000000.bin': new Blob(['stale same-device legacy body']),
        'recordings/missing.json': new Blob([JSON.stringify({
            format: 'math-reader-missing-recordings-v1',
            items: [{ id: missingId }, { id: missingId }, { id: incompleteId }]
        })]),
        'recordings/index.json': new Blob([JSON.stringify([{
            id: incompleteId,
            mimeType: 'audio/webm',
            byteLength: chunk.size,
            incomplete: true,
            chunks: [{ path: 'recordings/data/00000000.bin', size: chunk.size, sha256: 'chunk-hash' }]
        }])]),
        'recordings/data/00000000.bin': chunk
    };
    const events = [];
    const toasts = [];
    let restoredManifest = null;
    let restoredChunkText = null;
    const recordingStore = {
        remove: async id => { events.push('remove:' + id); },
        restore: async (manifest, readChunk) => {
            restoredManifest = manifest;
            restoredChunkText = await (await readChunk(0)).text();
            events.push('restore:' + manifest.id);
            return manifest;
        }
    };
    const { fn: importDataZip } = loadIsolatedFunction(importSource, 'importDataZip', {
        Blob,
        ArrayBuffer,
        appData: { settings: { preserved: true } },
        confirm: () => true,
        showToast: message => toasts.push(message),
        dataZipRead: async () => entries,
        dataZipText: async value => value instanceof Blob ? value.text() : String(value),
        pickApiSettingsFrom: settings => settings,
        mergeClassroomTombstones: value => value || {},
        saveFileData: async (id) => { events.push('save:' + id); },
        deleteFileData: async id => { events.push('delete:' + id); },
        initRecordingStore: () => {},
        recordingStore,
        window: { RecordingStorage: { FORMAT: 'recording-chunks-v1' } },
        _callNativeRecording: (method, id) => {
            events.push('native:' + method + ':' + id);
            return { ok: true };
        },
        sha256Hex: async () => 'chunk-hash',
        _findRecordingMaterial: id => id === incompleteId
            ? { material: incompleteMaterial }
            : null,
        saveExercisesToIDB: async () => {},
        saveData: () => true,
        restoreBlobsFromIDB: async () => {},
        applySettings: () => {},
        renderLibrary: () => {},
        renderNotesPage: () => {},
        renderClassroomPage: () => {},
    ...
    await importDataZip({ target: { files: [{}], value: 'selected' } });

    assert.match(toasts.at(-1), /^导入完成:/);
    assert.equal(restoredManifest.externalBackup, true,
        'incomplete state must survive startup recovery through the journal manifest');
    assert.equal(restoredChunkText, 'abc');
    assert.equal(incompleteMaterial.recordingStatus, 'recovered-incomplete');
    assert.equal(events.filter(event => event === 'remove:' + missingId).length, 1,
        'duplicate report entries must only clean a body once');
    assert.ok(events.includes('native:deleteRecording:' + missingId));
    assert.ok(events.includes('delete:classroom_' + missingId));
    assert.equal(events.includes('remove:' + incompleteId), false,
        'a conflicting missing report must not delete a recording included by metadata/index');
    assert.ok(events.indexOf('save:classroom_' + missingId) < events.indexOf('remove:' + missingId),
        'same-device leftovers must be removed after the generic IDB snapshot is restored');
    assert.ok(events.indexOf('restore:' + incompleteId) < events.indexOf('remove:' + missingId),
        'missing-body cleanup must wait until every portable body is verified');
});

test('ZIP import keeps old missing bodies when a portable chunk fails validation', async () => {
    const html = readIndexHtml();
    const importSource = sliceSource(html,
        'async function importDataZip(event)',
        'async function clearAllData()');
    const missingId = 'recording-missing-preserved';
    const corruptId = 'recording-corrupt-import';
    const chunk = new Blob(['bad'], { type: 'audio/webm' });
    const entries = {
        'metadata.json': new Blob([JSON.stringify({
            exportFormat: 'math-reader-full-zip-v2',
            classroom: { courses: [], seminars: [] }
        ...
        'recordings/index.json': new Blob([JSON.stringify([{
            id: corruptId,
            byteLength: chunk.size,
            chunks: [{ path: 'recordings/data/00000000.bin', size: chunk.size, sha256: 'expected-hash' }]
        }])]),
        'recordings/data/00000000.bin': chunk
    };
    const removed = [];
    ...
    const { fn: importDataZip } = loadIsolatedFunction(importSource, 'importDataZip', {
        ...
        initRecordingStore: () => {},
        recordingStore: {
            restore: async (_manifest, readChunk) => { await readChunk(0); },
            remove: async id => { removed.push(id); }
        },
        window: { RecordingStorage: { FORMAT: 'recording-chunks-v1' } },
        _callNativeRecording: (method, id) => {
            nativeDeletes.push(method + ':' + id);
            return { ok: true };
        },
        sha256Hex: async () => 'actual-corrupt-hash',
        _findRecordingMaterial: () => null,
        console: { warn: () => {}, error: () => {} }
    });

    await importDataZip({ target: { files: [{}], value: 'selected' } });

    assert.match(toasts.at(-1), /^导入失败: 录音备份分块校验失败/);
    assert.deepEqual(removed, [],
        'destructive cleanup must not run before every imported recording verifies');
    assert.equal(nativeDeletes.includes('deleteRecording:' + missingId), false);
});
